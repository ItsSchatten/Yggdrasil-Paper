    ...
    public static long getTimeFromString(@NotNull String string) {
        ...
        for (String s : strings) { // Loop through all strings split.
            ...
            if (Character.isUpperCase(character)) {
                if (character == 'M') {
                    time += Period.parse("P" + s).getMonths() * TimeUtils.TimeUnits.MONTH_MS.getAsLong(); // Gets the months from the provided string.
                    continue;
                }

                if (character == 'Y') {
                    time += Period.parse("P" + s).getYears() * TimeUtils.TimeUnits.YEAR_MS.getAsLong(); // gets the years from the provided string.
                    continue;
                }

                time += Period.parse("P" + s).getDays() * TimeUtils.TimeUnits.DAY_MS.getAsLong(); // Gets the days from the provided string.
            } else {
