...
public abstract class AbstractMenuInventory {
     ...
     *
     * @param slot The slot we should use to obtain this position.
    ...
    /**
     * Obtains an item based on an {@link InventoryPosition}.
     *
     * @param position The position we should use to obtain the item.
     ...
     * @return Returns <code>true</code> if the slot is taken by any item, <code>false</code> otherwise.
    ...
    public final void setRectangle(final int fromRow, @Range(from = 0, to = 8) final int fromColumn, final int toRow, @Range(from = 0, to = 8) final int toColumn, @NotNull final ItemCreator creator) {
        setRectangle(fromRow, fromColumn, toRow, toColumn, creator.makeForMenu());
    }
