package com.itsschatten.yggdrasil.libs.commands;
...
public abstract class CommandBase extends Command implements PluginIdentifiableCommand {
     ...
     * Subcommands.
     */
    private final Set<AbstractSubCommand> SUB_COMMANDS;
     ...
     *
     * @param subCommands The {@link AbstractSubCommand}(s) to register for this command.
     */
    public final void registerSubCommands(final AbstractSubCommand... subCommands) {
     ...
     *
     * @param name the name of the {@link AbstractSubCommand}
     */
    public final AbstractSubCommand getSubCommand(final String name) {
        ...
        if (commandSender instanceof ConsoleCommandSender || testPermission(commandSender)) {
            ...
            try {
                ...
                if (args.length >= 1) {
                    ...
                    final AbstractSubCommand subCommand = getSubCommand(args[0]);
    ...
    public boolean checkPermission(final IPermission permission) {
        ...
        tell(getPermissionMessage() == null ? StringUtil.color("<red>I'm sorry, but you do not have permission to perform this command.") : permissionMessage());
        return false;
    }
    ...
    /**
     * Overridden from super class to translate colors codes in message.
     *
    ...
    public final @Unmodifiable List<String> tabComplete(@NotNull CommandSender sender, @NotNull String alias, @NotNull String @NotNull [] args) throws IllegalArgumentException {
        ...
        if (args.length == 1) {
            ...
            tab.addAll(SUB_COMMANDS.stream().filter((subCommand) -> sender.hasPermission(subCommand.getPermission())).map(AbstractSubCommand::getId).filter((name) -> name.toLowerCase().contains(args[0].toLowerCase())).toList());
            SUB_COMMANDS.stream().filter((subCommand) -> sender.hasPermission(subCommand.getPermission())).map(AbstractSubCommand::getAliases)
                    .forEach((list) -> list.stream().filter((name) -> name.toLowerCase().contains(args[0].toLowerCase())).forEach(tab::add));
        }

        // Check if we are over 2 arguments.
        // If we are, attempt to find a sub command with args[0].
        if (args.length >= 2) {
            final AbstractSubCommand command = getSubCommand(args[0]);
